...
class Point2 {
	...
	public boolean equals(Object p) { 
		Point2 pp = (Point2) p; //p의 타입이 object이기 때문에 point2로 타입캐스팅 해줘야함
		
		return ix==pp.getIx() && iy==pp.getIy(); //ix, iy는 private으로 선언되어 있어 접근이 어렵기 때문에 게터로 접근해야함
...
//--- 생성자(constructor) ---//
	public objectStack(int cap) {
		//구현
		capacity=cap;
		top=0;
		try {
			data= new ArrayList<Point2>(cap);
