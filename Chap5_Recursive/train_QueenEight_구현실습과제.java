...
class Stack4 {
	...
	// --- 생성자(constructor) ---//
	public Stack4(int cap) {
		capacity = cap;
		top = 0;
		try {
			data = new ArrayList<Point>(cap);
...
public class train_QueenEight_구현실습과제 {
	...
	static void EightQueen(int[][] data) throws EmptyGenericStackException {
	...
	public static boolean checkRow(int[][] d, int crow) { // 배열 d에서 행 crow에 퀸을 배치할 수 있는지 조사
		for (int i = 0; i < d[0].length; i++) { // 모든 열을 검사
			if (d[crow][i] == 1) {
				return false;
			}
		}
	...
	public static boolean checkCol(int[][] d, int ccol) {// 배열 d에서 열 ccol에 퀸을 배치할 수 있는지 조사
		for (int i = 0; i < d.length; i++) { // 모든 행을 검사
			if (d[i][ccol] == 1) {
				return false;
			}
		}
	...
	public static boolean checkMove(int[][] d, int x, int y) {// (x,y)로 이동 가능한지를 check
		...
		if (checkRow(d, x) && checkCol(d, y) && checkDiagSE(d, x, y) && checkDiagSW(d, x, y)) { // boolean은 아무것도 안적을시 기본
																								// true
			return true;
