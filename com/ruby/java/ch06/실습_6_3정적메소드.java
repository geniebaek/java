...
class Student3 {
	 ...
	 * 
	 */
	public Student3(String name, int age, String subjects[], int scores[],int passFail[], int value) {
		this.name=name;
		this.age=age;
	...
	public void printStudentInfo() {
		System.out.println("이름 = "+name+", 나이 = "+age);
		for(int i= 0;i<subjects.length;i++) {
			System.out.println("과목"+(i+1)+"="+subjects[i]+"점수"+(i+1)+"="+scores[i]+"통과여부="+(passFails[i]<scores[i]?"pass":"fail"));
		}
	}
	/*
	 * 이름 = **, 나이 = **
	...
	public static void printAllStudents(Student[] students) {
		for(Student3 s : students) {
			for(int i = 0; i <  s.subjects.length;i++) {
				System.out.println(s.name+s.scores[i]+(s.isPassed()?"pass":"fail"));
				System.out.print("\t");
			}
			System.out.println("\n");
		}
	...
	public static void printSubjectStats(Student3[] students) {
		...
		System.out.println(System.out.println("최고점수="+s.max+"최저점수="+s.min);
	} 
}
