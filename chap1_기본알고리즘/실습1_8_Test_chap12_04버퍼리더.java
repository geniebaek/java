...
public class 실습1_8_Test_chap12_04버퍼리더 {
    ...
    public static String[] readSortFromFile(String filename) {
        ...
        try (BufferedInputStream in = new BufferedInputStream(new FileInputStream(filename));
            ...
            String[] stringArray = fileContent.trim().split("\\s+"); //trim()으로 맨 앞 뒤 공백을 제거하고 split()메서드로 공백을 기준으로 분리해서 배열에 집어넣음
    ...
    public static int[] convertSortToInt(String[] stringArray) {
    	int[] stringA=new int[stringArray.length];
    	for(int i = 0; i<stringArray.length;i++) {
    		stringA[i]=Integer.parseInt(stringArray[i]);
    	}
    	Arrays.sort(stringA);
    	return stringA;
    }
